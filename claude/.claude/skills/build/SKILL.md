...
4. **Commit** (approval-gated): re-verify branch + cwd, propose the atomic
   commit per the repo's convention. In stage-only repos: stage, never
   commit.
...
- A task trending well past ~300 changed lines may warrant a split proposal
  **before** it grows — suggest it, but a bigger PR is fine if it stays
  coherent.
